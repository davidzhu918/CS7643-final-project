"""
This module re-implements panoptic_seg.py in detectron2/projects/panopitc_deeplab/panoptic_seg.py 
which modifies PanopticDeepLab to use single ASPP head.

To use this, replace detectron2/projects/panopitc_deeplab/panoptic_seg.py with the content of this
file, and proceed with detectron2 training.
"""
import numpy as np
...
class PanopticDeepLab(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.aspp_head = build_aspp_head(cfg, self.backbone.output_shape())
        self.sem_seg_head = PanopticDeepLabSemSegHead(self.aspp_head)
        self.ins_embed_head = PanopticDeepLabInsEmbedHead(self.aspp_head)
...
def build_aspp_head(cfg, input_shape):
    """
    Build a instance embedding branch from `cfg.MODEL.SEM_SEG_HEAD.NAME`.
    """
...
class PanopticDeepLabASPPHead(DeepLabV3PlusHead):
    """
    ...
    def losses(self, predictions, targets, weights=None):
        ...
        losses = {"loss_aspp": loss * self.loss_weight}
        return losses
...
class PanopticDeepLabSemSegHead(nn.Module):
    ...
    def __init__(self, aspp_head):
        super().__init__()
        self.aspp_head = aspp_head

        self.input_shape = aspp_head.input_shape
        self.decoder_channels = aspp_head.decoder_channels
        self.norm = aspp_head.norm
        self.head_channels = aspp_head.head_channels
        self.loss_weight = aspp_head.loss_weight
        self.loss_type = aspp_head.loss_type
        self.loss_top_k = aspp_head.loss_top_k
        self.ignore_value = aspp_head.ignore_value
        self.num_classes = aspp_head.num_classes
        self.use_depthwise_separable_conv = aspp_head.use_depthwise_separable_conv
        self.decoder_only = aspp_head.decoder_only
        self.common_stride = aspp_head.common_stride
        ...
        if self.use_depthwise_separable_conv:
            ...
            self.head = nn.Sequential(
                Conv2d(
                    ...
                    padding=1,
                    bias=use_bias,
                    norm=get_norm(self.norm, self.decoder_channels[0]),
                    activation=F.relu,
                ),
                Conv2d(
                    self.decoder_channels[0],
                    ...
                    padding=1,
                    bias=use_bias,
                    norm=get_norm(self.norm, self.head_channels),
                    activation=F.relu,
                ),
            )
            weight_init.c2_xavier_fill(self.head[0])
            weight_init.c2_xavier_fill(self.head[1])
        self.predictor = Conv2d(self.head_channels, self.num_classes, kernel_size=1)
        ...
        if self.loss_type == "cross_entropy":
            ...
            self.loss = DeepLabCE(ignore_label=self.ignore_value, top_k_percent_pixels=self.loss_top_k)
        else:
    ...
    def layers(self, features):
        assert self.decoder_only
        y = self.aspp_head.layers(features)
        y = self.head(y)
        y = self.predictor(y)
        return y
...
class PanopticDeepLabInsEmbedHead(nn.Module):
    ...
    def __init__(self, aspp_head):
        ...
        self.center_predictor = Conv2d(self.head_channels, 1, kernel_size=1)
        ...
        self.offset_predictor = Conv2d(self.head_channels, 2, kernel_size=1)
